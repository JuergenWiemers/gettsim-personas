"""
Persona for a couple with 5 children receiving basic subsistence benefits.
...
@persona_description(
    description=(
        ...
        Jointly taxed married couple with five children aged 3, 5, 7, 9, and 11.
...
def alter() -> np.ndarray:
    return np.array([30, 30, 3, 5, 7, 9, 11])
...
def arbeitsstunden_w() -> np.ndarray:
    return np.array([15, 0, 0, 0, 0, 0, 0])
...
def einkommensteuer__abzüge__kinderbetreuungskosten_m() -> np.ndarray:
    return np.array([0, 0, 100, 100, 0, 0, 0])


@persona_input_element(start_date="2005-01-01")
def einkommensteuer__abzüge__p_id_kinderbetreuungskostenträger() -> np.ndarray:
    return np.array([-1, -1, 0, 0, -1, -1, -1])


@persona_input_element(start_date="2005-01-01")
def einkommensteuer__einkünfte__aus_forst_und_landwirtschaft__betrag_y() -> (
    np.ndarray
):
    return np.array([0, 0, 0, 0, 0, 0, 0])
...
def einnahmen__bruttolohn_m() -> np.ndarray:
    return np.array([1000, 0, 0, 0, 0, 0, 0])
...
@persona_input_element()
def sozialversicherung__kranken__beitrag__bemessungsgrundlage_rente_m() -> (
    np.ndarray
):
    return np.array([0, 0, 0, 0, 0, 0, 0])
